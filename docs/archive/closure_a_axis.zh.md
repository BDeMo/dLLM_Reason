...
**状态**：A 轴 inference-time 粒度扫描 **post-closure 全量结果出齐**。A6 gen_length 是最强单轴信号（20%），H3 最终 SUPPORTED（23.3%），全方法 union 15/18=83.3%。
...
| **H3** | pass@N at temperature，判 capacity ceiling | 30 (fail) | 7/30 rescue=23.33%；idx=0,2,8,13,15,24,28；4 stuck | **SUPPORTED** |
...
gen_length ──── A6       SUPPORTED (12/60 = 20.00%) ← 最强单轴
A4×A5 joint ── 6-cell   16.67% (10/60, overlap 预测完美验证)
pass@N ──────── H3       SUPPORTED (7/30 = 23.33%, capacity ceiling REJECTED)

rescue signal ↗ 随粒度变粗而变强；A6 是甜点
全方法 union = 15/18 = 83.3%，只剩 3 条 true capacity ceiling
```
...
### 4.6 H3 最终 SUPPORTED（23.3%），推翻早期预判

H3 早期观察（17 条）预判 INCONCLUSIVE。**最终结果大逆转**：30 条 fail 中 7 条 rescue（idx=0,2,8,13,15,24,28），rescue_rate=**23.33%** → **SUPPORTED**。4 条 stuck（pass@8=0 at all temps）。

**解读**：capacity ceiling 假设被 REJECTED —— 模型不是"根本不会"，而是"默认配置下做不对"。diversity sampling 本身是有效杠杆，但 A 轴的更便宜旋钮（特别是 A6 gen_length）能达到类似效果。

**注意**：p5_h3_crossref.py 输出 h3_rescue=0 是 **BUG**（H3 用 fail_XXXX.json 命名，不是 XXXX.json），真实 H3 rescue=7，交叉结果为手动计算。

### 4.7 全方法 union 15/18 = 83.3%，只剩 3 条 true capacity ceiling
