...
**日期**：2026-04-15（live，H3 结果出来会继续更新）
...
DAG search 三次独立实现 + 两次 token 级 revise 实验全部 0 rescue 之后，A 轴在粗粒度层出三个正信号：A4 block-layout rerank（5/60 = 8.33%）、A5 prompt-template rerank（8/60 = 13.33%）、**A6 gen-length rerank（12/60 = 20.00%，最强单轴）**。H3 pass@N 最终 SUPPORTED（7/30 = 23.33%）。全方法 union 15/18 = 83.3%，只剩 3 条 true capacity ceiling。
...
| **H3 · pass@N** | **采样多样性** | **—** | **SUPPORTED** | **7/30 (23.33%)** |
...
**注意**：`p5_h3_crossref.py` 输出 h3_rescue=0 是 **BUG**（H3 用 `fail_XXXX.json` 命名，不是 `XXXX.json`）。真实 H3 rescue=7，交叉结果为手动计算。

**解读**：H3 SUPPORTED（fail_p@8 > 20%）意味着 capacity ceiling 假设被 **REJECTED** —— 模型不是"根本不会"，而是"默认配置下做不对"。diversity sampling（23.33%）是有效杠杆，但 A 轴更便宜的旋钮（A6=20%）能达到类似效果。

**H3 rescue 集交叉**：
- H3 独有：{2, 24} —— 这 2 条只有 diversity sampling 能救，A 轴旋钮覆盖不到
- H3 ∩ A6 独有：{0} —— 只有 H3 和 A6 能救
- H3 与 A4∪A5 共享：{8, 13, 15, 28} —— A 轴旋钮也能覆盖
...
| gen_length | A6 | 换生成长度 | **信号**（20.00%，最强） |
| layout × template | A4×A5 joint | 6-cell ensemble | 16.67%（overlap 完美验证） |
| sampling scheme | H2, H3 | H2 换 block_length / H3 换 T+N | H3 **SUPPORTED**（23.33%） |

信号首次出现在 **block 级**，到 prompt/gen_length 级更强。更细的（token, span, edge）全 dead。A6 gen_length 是最强单轴信号（20%），g160 单点已接近 A5 的 any-template ensemble。

**全方法 union = 15/18 = 83.3%**。只剩 3 条（idx=5,6,16）所有方法都救不了 = true capacity ceiling。不同维度基本正交：A6 独有 {19,51}、H3 独有 {2,24}。
...
A6 SUPPORTED (12/60) ───┘     → A6 最强（20%），g160 是甜点
                              ...
                              → 全方法 union 15/18 = 83.3%

H3 SUPPORTED (7/30 = 23.33%)
  → capacity ceiling REJECTED（模型能力没到上限）
  → H3 独有 {2,24}，说明 diversity sampling 有 A 轴覆盖不到的独立信号
...
2. ~~H3 收尾~~ → **DONE**：SUPPORTED, 7/30=23.33%。H3 独有 {2,24}
...
4. **p5_h3_crossref.py BUG**：H3 用 `fail_XXXX.json` 命名导致脚本匹配不到。手动算了交叉，但脚本需要修。
